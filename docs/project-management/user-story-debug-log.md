...
| US-016 Mise a jour statut employe | Changer le statut sans historique ferait perdre la tracabilite. | Un simple UPDATE ne suffit pas. | Mise a jour du statut courant, insertion dans `commande_statuts`, notifications email sur `terminee` et `en_attente_retour_materiel`, blocage des commandes deja cloturees. | "Chaque changement garde qui l'a fait et quand ; les statuts sensibles previennent aussi le client." |
| US-017 Annulation employe avec motif | Risque d'annuler sans justification en choisissant directement `annulee` dans le select statut. | Le sujet demande un contact client et un motif, donc l'annulation ne doit pas passer par le formulaire generique de statut. | Retrait de `annulee` des formulaires d'action statut, rejet serveur dans `changeStatusByEmployee`, annulation reservee a `cancelByEmployee`, commandes cloturees non annulables. | "L'annulation interne passe obligatoirement par le mode de contact et le motif." |
...
| US-019 Gestion menus | Version admin incomplete si les menus ne peuvent pas choisir leurs plats. | La table `menu_plats` existait deja mais n'etait pas encore pilotee par l'admin. | Ajout des cases a cocher plats dans `/admin/menus` et synchronisation de `menu_plats`. | "L'admin gere maintenant la composition d'un menu sans migration de base." |
| US-020 Gestion plats | Les allergenes etaient visibles dans la base mais pas administrables. | La table `plat_allergenes` existait deja mais le CRUD plat ne l'utilisait pas. | Ajout des allergenes dans `/admin/plats`, affichage du resume et synchronisation de `plat_allergenes`. | "Les plats restent reutilisables et portent leurs allergenes depuis l'espace admin." |
...
| US-022 Comptes employes | L'enonce demande que l'admin fournisse email et mot de passe, sans l'envoyer par email. | L'ancien flux generait un mot de passe temporaire cote serveur et orientait vers la reinitialisation. | Ajout du mot de passe et de sa confirmation dans `/admin/employes`, hash en base, notification email sans mot de passe, activation/desactivation reservee admin. | "L'admin definit le mot de passe, mais l'email ne transporte jamais ce secret." |
| US-023 Dashboard admin | Les statistiques doivent fonctionner avec MongoDB, mais rester consultables si MongoDB est indisponible. | L'environnement local peut avoir `mongosh` disponible sans extension PHP MongoDB. | Lecture des agregats via `mongosh`, affichage de la source et secours SQL transparent. | "MongoDB sert les agregats du dashboard, le SQL reste seulement un secours local." |
...
| US-025 CA par menu/periode | Il fallait prouver que le dashboard lit bien une base non relationnelle. | Les scripts MongoDB etaient presents, mais la page ne rendait pas la source evidente. | Aggregation MongoDB via `mongosh`, source affichee sur `/admin/statistiques`, filtre menu/periode conserve et fallback SQL si MongoDB tombe. | "La page admin montre la source MongoDB et reste robuste en local." |
| US-026 Accessibilite RGAA | Il fallait une trace plus solide qu'une simple mention "labels et alt". | Une conformite totale ne peut pas etre declaree sans audit exhaustif. | Audit interne sur 16 pages, ajout skip-link, captions, reduced-motion et documentation des reserves clavier/contrastes/lecteur d'ecran. | "Je peux prouver les bases RGAA sans pretendre a une conformite totale non verifiee." |
...
| Les pages legales ne ressemblaient pas aux frames Figma. | Elles utilisaient encore la vue placeholder generique avec un simple panneau `Information`, puis un encadre interne `Point cle UX` etait reste visible. | Ajout d'un rendu public legal parametrable : titre, intro, carte blanche, lignes de contenu finalisees, sans note interne visible. | "Le controleur prepare les textes, la vue affiche un composant reutilisable, le footer reste commun." |
...
| Le bouton `Tous les filtres` fonctionnait sur `/menus` mais pas sur l'accueil. | L'accueil avait seulement les filtres rapides locaux, sans attribut d'ouverture ni overlay partage. | Creation d'un partial `menu-filter-overlay`, reutilisation sur les deux pages, ajout des donnees de filtrage avance sur les cartes accueil et adaptation du JavaScript a un conteneur non-formulaire. | "Quand deux pages ont le meme bouton, elles doivent partager le meme composant et le meme comportement." |
| Le filtre overlay `Stock limite` ne renvoyait aucun resultat apres avoir utilise le filtre rapide `Disponible`. | Les deux filtres se cumulaient : le site cherchait des menus a la fois `Disponible` et `Stock limite`, ce qui est contradictoire. | Le choix de disponibilite dans l'overlay remet le filtre rapide sur `Tous`; si l'utilisateur reclique `Disponible`, le choix avance de disponibilite est nettoye. | "Les filtres visibles doivent eviter les contradictions cachees : un choix avance de disponibilite doit remplacer le filtre rapide equivalent." |
| Le bouton `Reinitialiser` de l'overlay filtres gardait le panneau ouvert. | Le JavaScript nettoyait bien les champs et les cartes, mais ne fermait pas l'overlay apres l'action. | Ajout de `closeFilterOverlay()` apres la remise a zero, puis test sur `/` et `/menus`. | "Reinitialiser doit ramener l'utilisateur a la page avec tous les resultats visibles, pas seulement vider les champs dans le panneau." |
| Les photos de menus etaient trop petites pour inspecter les plats. | Les images etaient statiques dans les cartes et les pages detail, sans apercu agrandi. | Ajout d'un lightbox partage dans le layout, boutons d'apercu sur accueil, liste menus, image principale detail et galerie, fermeture par fond, croix ou touche Echap. | "Une photo importante pour choisir un menu doit pouvoir etre consultee en grand sans quitter la page." |
...
| La page connexion Figma montrait des boutons `Client`, `Employé`, `Admin`, mais le backend n'attend pas de champ role au login. | Le role de l'utilisateur est deja determine par le compte trouve en base, pas par un choix dans le formulaire. Le selecteur creait une confusion visuelle. | Suppression du selecteur, de la carte role et du JavaScript associe. Le formulaire demande seulement email et mot de passe. | "Je ne laisse pas l'utilisateur choisir son role côté formulaire ; le role reste une propriete securisee du compte." |
...
| Les images de galerie etaient floues une fois ouvertes en grand. | Les anciennes miniatures `thumb-*.png` etaient des fichiers de 128x100 px prevus pour un affichage compact, mais l'overlay les agrandissait. | Generation d'une image haute definition par plat, rangement par menu dans `public/images/menu-details/<menu>/dish-*.png`, puis recablage de `MenuPresentation` pour que chaque plat ouvre une image nette. | "Une miniature ne doit pas servir de source a l'agrandissement : l'overlay doit pointer vers un asset haute definition." |
